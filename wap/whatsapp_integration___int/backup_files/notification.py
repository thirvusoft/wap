...
"""
	...
	def whatsapp(self, doc, context): 
		providers=frappe.db.get_value('WAP Provider',{"name":self.custom_wap_provider} , ['api_end_point', 'api_authentication'])
		conn = http.client.HTTPSConnection(providers[0])
		receiver_list=self.get_receiver_list(doc, context)
		fields=[]
		doc_fields=frappe.db.get_all("Doc Value Mapping", filters={"parent":self.name}, pluck="fields", order_by="idx")
		for field in doc_fields:
			fields.append(doc.get(field))
		for contact in range(0, len(receiver_list), 1):
			payload = json.dumps({
				"countryCode": "+91",
				"phoneNumber": receiver_list[contact],
				"callbackData": "some text here",
				"type": "Template",
				"template": {
					"name": self.custom_whatsapp_message_template,
					"languageCode": "en_US",
					"headerValues": [],
					"bodyValues":fields,
					"buttonValues": {
			...
			try:
				headers = {
				'Authorization': get_decrypted_password(providers[1]),
				...
				print(res.read(), payload)
				data = res.read()
				w_doc = frappe.new_doc("WAP API Log")
				w_doc.update({
						"mobile_no": receiver_list[contact],
						"request":f"{payload}",
						"user":frappe.session.user(),
						"request_method":"POST",
						"status":"Success",
						"response" : res.read(),
						"datetime": frappe.utils.get_datetime()
					})
				w_doc.flags.ignore_permissions = True
				w_doc.flags.ignore_mandatory = True
				w_doc.insert()
